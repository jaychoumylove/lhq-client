...
export default {
		...
		uni.login({
			success: res => {
				this.request(this.API.AUTH_LOGIN, {
					...
					referrer: this.getData('referrer'),
					code: res.code,
				}, res => {
	...
	invokeSocket() {
		if (!this.socketTask || this.socketTask.readyState == 2 || this.socketTask.readyState == 3) {
			this.socketTask = uni.connectSocket({
				url: this.WSS_URL,
				success: res => console.log('WebSocket 连接成功！', res)
			})

			this.socketTask.onMessage(res => {
				// console.log('收到 WebSocket消息', res);
				if (res.hasOwnProperty('data') == false||!res.data) return;
				const msg = JSON.parse(res.data)
				// console.log('收到 WebSocket消息MSG', msg);
				const {
					type,
					data
				} = msg
				...
				switch (type) {
					...
					case 'sendHot': // 打榜弹幕
						page && page.$refs.guildComponent &&
							page.$refs.guildComponent.sendHotDanmaku && page.$refs.guildComponent.sendHotDanmaku(data)
						break;
					case 'sendItem': // 送礼物
						page && page.$refs.guildComponent &&
	...
	/**通用分享*/
	commonShareAppMessage(shareType,otherparam=null, shareData = null) {
		// -----默认配置
		// 默认跳转到首页
		// referrer上级
		...
		// 默认使用明星分享海报，没有则使用默认
		let imageUrl = this.getData('config').share_img

		// ----- 使用特殊的分享配置
		if (shareType) {
			if (typeof shareType == 'string' || typeof shareType == 'number') {
				shareType = this.getData('config').share[shareType]
			}
			if (shareType.title) title = shareType.title
			if (shareType.path) path += `&${shareType.path}`
			if (shareType.imageUrl) imageUrl = shareType.imageUrl
		}
		if (otherparam) path += escape(`?${otherparam}`)
		
		// 强制使用分享数据
		if (!!shareData) {
			if (shareData.hasOwnProperty('image')) imageUrl = shareData.image;
			if (shareData.hasOwnProperty('title')) title = shareData.title;
			if (shareData.hasOwnProperty('path')) path += `&${shareData.path}`;
			if (shareData.hasOwnProperty('otherparam')) path += escape(`?${shareData.otherparam}`);
		}
